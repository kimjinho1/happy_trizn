defmodule HappyTrizn.Games.Skribbl do
  ...
  # 한국어 단어 풀 — 일상 / 게임 / 만화애니 / 밈 / MZ 슬랭. 개발자 + 많은 사람이 알 만한.
  @word_pool ~w(
    ...
    엘든링 젤다 마리오 포켓몬 동물의숲 어몽어스 테트리스 갤럭시안

    진격의거인 원피스 나루토 드래곤볼 슬램덩크 짱구 도라에몽 짱구는못말려
    슬레이어즈 코난 데스노트 주술회전 귀멸의칼날 체인소맨 장원영 뉴진스
    스파이패밀리 원펀맨 헌터헌터 베르세르크 하이큐 이누야사 에반게리온
    아이언맨 스파이더맨 캡틴아메리카 헐크 토르 배트맨 슈퍼맨 원더우먼

    개발자 코딩 깃허브 스택오버플로우 커밋 머지 리팩토링 디버깅 빌드 배포
    리눅스 도커 쿠버네티스 자바스크립트 파이썬 엘릭서 러스트 타입스크립트
    리액트 백엔드 프론트엔드 데이터베이스 알고리즘 함수 변수 클래스 객체

    킹받네 갓생 럭키비키 어쩔티비 저쩔티비 알잘딱깔센 무야호 갑분싸 인싸 아싸
    꾸안꾸 핵노잼 띵작 존맛탱 가성비 뇌피셜 영혼없다 웃프다 멘붕 광탈
    소확행 워라밸 욜로 라떼는 꼰대 현타 자만추 스불재 빠태 별다줄

    아이돌 콘서트 콜라 떡볶이 노래방 PC방 한강 서울 부산 제주도
