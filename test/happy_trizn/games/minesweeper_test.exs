defmodule HappyTrizn.Games.MinesweeperTest do
  ...
  describe "init/1" do
    test "10x10 grid, 모든 cell hidden, mines_placed false" do
      ...
      assert Enum.all?(state.cells, fn {_, c} -> not c.revealed and not c.flagged and not c.mine end)
  ...
  describe "first reveal" do
    test "첫 클릭 시 mines 생성됨 + 클릭 위치 안전" do
      {:ok, state} = Minesweeper.init(%{})
      {:ok, new_state, _} = Minesweeper.handle_input("p1", %{"action" => "reveal", "r" => 5, "c" => 5}, state)
  ...
  describe "flag/2" do
    test "hidden cell 에 flag 토글" do
      {:ok, state} = Minesweeper.init(%{})
      {:ok, s1, _} = Minesweeper.handle_input("p1", %{"action" => "flag", "r" => 0, "c" => 0}, state)
    ...
    test "이미 revealed 셀에는 flag 안 됨" do
      {:ok, state} = Minesweeper.init(%{})
      {:ok, s1, _} = Minesweeper.handle_input("p1", %{"action" => "reveal", "r" => 5, "c" => 5}, state)
      # (5,5) 는 revealed
  ...
  describe "lose / win" do
    test "mine 클릭 → over=:lose" do
      ...
      {:ok, s1, _} = Minesweeper.handle_input("p1", %{"action" => "reveal", "r" => 0, "c" => 0}, state)

      # mine 위치 찾기
      {mr, mc} = s1.cells |> Enum.find(fn {_, c} -> c.mine end) |> elem(0)
      {:ok, s2, _} = Minesweeper.handle_input("p1", %{"action" => "reveal", "r" => mr, "c" => mc}, s1)

      assert s2.over == :lose
      # 모든 mine reveal 됨
      mines_revealed = s2.cells |> Enum.filter(fn {_, c} -> c.mine end) |> Enum.all?(fn {_, c} -> c.revealed end)
    ...
    test "모든 안전 셀 reveal → over=:win" do
      ...
      revealed_all_safe =
        ...
        |> Enum.reduce(s1, fn {{r, c}, _}, acc ->
          {:ok, new, _} = Minesweeper.handle_input("p1", %{"action" => "reveal", "r" => r, "c" => c}, acc)
          new
        end)
  ...
  describe "out of bounds" do
    test "잘못된 좌표는 무시" do
      {:ok, state} = Minesweeper.init(%{})
      {:ok, ^state, []} = Minesweeper.handle_input("p1", %{"action" => "reveal", "r" => -1, "c" => 0}, state)
      {:ok, ^state, []} = Minesweeper.handle_input("p1", %{"action" => "reveal", "r" => 100, "c" => 0}, state)
    end
  end
end
