...
## 만든 이유
- 42서울 할 때 4명이 5주 동안 겨우 만들었던 채팅/게임 웹이, 코드 한 줄도 치지 않고 클로드로 만들어지는지 확인하고 싶었음  
- 하루만에 완성뿐만 아니라 더 많은 게임이 구현됐고, 테스트 코드 커버리지도 높음.  
- BE, FE 프레임워크를 처음 봤을 때라 그런 것도 있지만, 대략 4명 * 25일이라 생각하면 100인데 하루 만에 만들었으니, 능률이 그때의 나보다 100배인 샘.  
- 개발자가 하는 일이 이런 장난감을 만드는 것은 아니지만 ai의 개발 생산 속도를 봤을 때,
- 앞으로 도메인 지식을 쌓지 않으면 나같은 주니어 개발자는 조만간 필요없을 것이라는 확신이 든다.

---
 
- 디자인 문서: [docs/DESIGN.md](docs/DESIGN.md)
- 게임별 본격 spec (Jstris 수준): [docs/GAMES_SPEC.md](docs/GAMES_SPEC.md)
- **Trizmon (자체 IP 몬스터 RPG, 장기 flagship) spec: [docs/TRIZMON_SPEC.md](docs/TRIZMON_SPEC.md)**
- 테스트 계획: [docs/TEST_PLAN.md](docs/TEST_PLAN.md)
- 디자인 감사 (스냅샷): [docs/DESIGN_AUDIT.md](docs/DESIGN_AUDIT.md)
- AI 에이전트 가이드: [docs/AGENTS.md](docs/AGENTS.md)
- 게임 라인업: Tetris ✅, 캐치마인드 ✅, Bomberman ✅, Snake.io ✅, 2048 ✅, 지뢰찾기 ✅, Pac-Man ✅, 스도쿠 ✅
- 테마: ☀️ Light+ · 🌙 Dark+ · 🦉 Night Owl · 💻 Hacker Terminal — `/settings/games` 상단에서 즉시 전환
...
향후 ([docs/DESIGN.md](docs/DESIGN.md) 참고):
...
- [x] **Sprint 4i**: 테마 시스템 4종 + 모달 브릿지 + 게임 시각 픽스 — Light+ / Dark+ / Night Owl / Hacker Terminal daisyUI 플러그인 / `/settings/games` 테마 picker / Bomberman 채팅 게임 grid 높이 fill + 폭발 자동 클리어 (`:state_changed` broadcast) + 폭탄/플레이어 z-order 픽스 / Snake.io 채팅 캔버스 높이 fill / Tetris 셀 inset bevel (같은 색 인접 경계) / top-nav ⚙️ 옵션 모달 브릿지 (게임 중 페이지 이동 X)

### TODO (앞으로 할 일)

- [ ] **Sprint 5c (장기 flagship)** — Trizmon 자체 IP 몬스터 RPG. 모험 / PvE / PvP 3 모드. 18 타입 / 100+ 종 / 진화 / 도감 / AI 생성 이미지. spec: [docs/TRIZMON_SPEC.md](docs/TRIZMON_SPEC.md). Sprint 5c-1 부터 단계 진행
...
자세한 게임별 진행 = [docs/GAMES_SPEC.md](docs/GAMES_SPEC.md), 테스트 = [docs/TEST_PLAN.md](docs/TEST_PLAN.md).
